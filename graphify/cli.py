...
def _prune_graph_json_sources(graph_path: Path, stale_sources: list[str]) -> int:
    """
    Remove graph data associated with stale source files from ``graph.json`` in place.
    
    Parameters:
        graph_path (Path): Path to the graph file.
        stale_sources (list[str]): Source-file identifiers whose nodes and relationships should be removed.
...
def dispatch_command(cmd: str) -> None:
    """
    Dispatch a graphify CLI subcommand using arguments from ``sys.argv``.
    
    Parameters:
    	cmd (str): The subcommand to execute.
